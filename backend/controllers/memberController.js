    ...
    if (memberRec.length === 0) {
      ...
      await db.query(
        `INSERT INTO members 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
        [groupId, userName, "", "", "", joinDate, adminId, userId],
      );
    }

    res.json({ message: "User added as member successfully" });
  } catch (error) {
...
// NEW: Get member_id for the logged-in user in a specific group
