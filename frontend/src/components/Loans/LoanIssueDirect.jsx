...
const LoanIssueDirect = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post("/loans/issue", { ...formData, groupId });
      toast.success("Loan issued successfully");
      navigate("/loans");
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to issue loan");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Issue Loan Directly (Admin)</h1>
...
export default LoanIssueDirect;
